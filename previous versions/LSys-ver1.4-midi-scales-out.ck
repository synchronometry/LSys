    ...
    while( true )
    {
        ...
        while( min.recv(msg) )
        {
            ...
            for( 0 => int i; i < str.length(); i++)
            {
                ...
                if( str.charAt( i ) == 'B' ) 
                {
                    if( i < axiom_scale.size())
                    {
                        msg.data2 + axiom_scale_mout[i] => msg.data2;
                        mout.send(msg); 
                    }
                    else
                    {
                        (i % axiom_scale_mout.size()) => int moduloNote;
                        msg.data2 + axiom_scale_mout[moduloNote] => msg.data2;
                        mout.send(msg); 
                    }
                }
            }
